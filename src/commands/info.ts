...
import { runBrewCommandToOutput, execBrew, getCurrentFormulaName } from '../utils/brewUtils';

export async function info(outputChannel: vscode.OutputChannel, node?: any) {
    let formula = node ? node.label : getCurrentFormulaName(vscode.window.activeTextEditor);
    if (!formula) {
        formula = await vscode.window.showInputBox({ prompt: 'Enter formula for info' });
    }
    if (formula) {
        ...
        runBrewCommandToOutput(`brew info ${formula}`, `Fetching info for ${formula}`, outputChannel);
    }
}

export async function depsTree(outputChannel: vscode.OutputChannel, node?: any) {
    let formula = node ? node.label : getCurrentFormulaName(vscode.window.activeTextEditor);
    if (!formula) {
        formula = await vscode.window.showInputBox({ prompt: 'Enter formula for dependency tree' });
    }
    if (formula) {
        runBrewCommandToOutput(`brew deps --tree ${formula}`, `Fetching dependency tree for ${formula}`, outputChannel);
    }
}

export async function openTap(node?: any) {
    if (!node || !node.label) {
        vscode.window.showErrorMessage('Please select a tap from the list to open.');
        return;
    }
        ...
        .then(stdout => {
            try {
                ...
                if (info && info.length > 0 && info[0].path) {
                    ...
                    vscode.commands.executeCommand('vscode.openFolder', vscode.Uri.file(tapPath), true);
                } else {
                    vscode.window.showErrorMessage(`Could not find path for tap ${tapName}`);
                }
            } catch (e: any) {
