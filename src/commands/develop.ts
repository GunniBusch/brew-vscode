...
export async function create() {
    const url = await vscode.window.showInputBox({ prompt: 'Enter URL to create formula from' });
    if (url) {
...
export async function test() {
    ...
    if (!formula) {
        formula = await vscode.window.showInputBox({ prompt: 'Enter formula to test' });
    }
    if (formula) {
        const terminal = vscode.window.createTerminal({
            ...
            .catch(({ stderr }) => {
                ...
                execBrew(`brew ruby -e "${rubyExpectedCask}"`)
                    ...
                    .catch(({ stderr: stdCask }) => {
                         vscode.window.showErrorMessage(`Could not find source for ${formula}: ${stderr} ${stdCask}`);
                    });
            });
    }
}
