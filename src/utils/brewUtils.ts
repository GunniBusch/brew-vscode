 ...
 */
export function execBrew(command: string, options: ExecOptions = {}): Promise<string> {
 ...
 */
export function getCurrentFormulaName(editor: vscode.TextEditor | undefined): string | undefined {
    if (editor) {
        const path = require('path');
        return path.basename(editor.document.fileName, path.extname(editor.document.fileName));
    }
    return undefined;
}
